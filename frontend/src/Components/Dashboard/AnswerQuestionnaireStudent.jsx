...
import { useStudentQuestionnaireDetails } from "../../hooks/useStudentQuestionnaireDetails";
...
import { CheckCircle, BookOpen, Send, AlertTriangle, GraduationCap } from "lucide-react";
import SidebarStudent from "./SidebarStudent";

export default function AnswerQuestionnaireStudent() {
  const { templateCode } = useParams();
  const { template, loading } = useStudentQuestionnaireDetails(templateCode);
  const { submitAnswers } = useStudentQuestionnaires();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate("/etudiant/dashboard?submitted=true");
      }, 3000);
    } catch (error) {
        ...
        <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-purple-50 to-indigo-50 p-4">
          <div className="text-center bg-white p-8 lg:p-12 rounded-2xl shadow-2xl max-w-md w-full">
            ...
            <button
              ...
              className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 lg:px-6 lg:py-3 rounded-lg font-medium transition-colors text-sm lg:text-base"
            >
      ...
      <div className="min-h-screen flex lg:flex-row flex-col">
        ...
        <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-purple-50 to-indigo-50 p-4">
          <div className="text-center bg-white p-8 lg:p-12 rounded-2xl shadow-2xl max-w-lg w-full">
            <div className="w-20 h-20 lg:w-24 lg:h-24 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-4 lg:mb-6">
              <CheckCircle className="w-10 h-10 lg:w-12 lg:h-12 text-purple-600" />
            </div>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 mb-6">
              ...
              <p className="text-purple-600 text-xs lg:text-sm mt-1">
                Vous serez automatiquement redirigé dans quelques secondes
              </p>
            </div>
            <button
              onClick={() => navigate("/etudiant/dashboard?submitted=true")}
              className="bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white px-4 py-2 lg:px-6 lg:py-3 rounded-lg font-medium transition-all duration-200 shadow-lg hover:shadow-xl text-sm lg:text-base"
            >
          ...
          <div className="bg-white shadow-lg border-b border-slate-200 p-4 lg:p-6 sticky top-0 z-40">
            ...
            <div>
              ...
              <div className="w-full bg-slate-200 rounded-full h-2 lg:h-3 overflow-hidden">
                <div 
                  className="bg-gradient-to-r from-purple-500 to-indigo-600 h-2 lg:h-3 rounded-full transition-all duration-700 ease-out shadow-sm"
            ...
            <div className="max-w-4xl mx-auto space-y-6 lg:space-y-8">
              ...
              <div className="bg-white rounded-xl lg:rounded-2xl shadow-xl p-6 lg:p-8 border border-slate-100">
                <div className="flex flex-col sm:flex-row items-start space-y-4 sm:space-y-0 sm:space-x-6">
                  <div className="bg-gradient-to-br from-purple-500 to-indigo-600 p-3 lg:p-4 rounded-xl lg:rounded-2xl shadow-lg flex-shrink-0">
                    <FaGraduationCap className="w-6 h-6 lg:w-8 lg:h-8 text-white" />
                  </div>
                  <div className="flex-1">
                    ...
                    <p className="text-slate-600 leading-relaxed text-base lg:text-lg">
                      Merci de prendre le temps de répondre à ce questionnaire d'évaluation. Vos réponses sont importantes pour améliorer la qualité de la formation.
                    </p>
                    {template.filiereCode && (
                      <div className="mt-3">
                        <span className="inline-flex items-center px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-sm font-medium">
                          Formation: {template.filiereCode}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              {template.sections.map((section, sectionIndex) => (
                <div key={section.id} className="bg-white rounded-xl lg:rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
                  {/* Section Header */}
                  <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-4 lg:p-6 border-b border-slate-200">
                    <div className="flex items-center space-x-4">
                      <div className="bg-purple-100 text-purple-600 w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center font-bold text-base lg:text-lg flex-shrink-0">
                        {sectionIndex + 1}
                      </div>
                        ...
                        className={`p-4 lg:p-6 rounded-lg lg:rounded-xl border-2 transition-all duration-300 ${
                          validationErrors[question.id] 
                            ...
                            : responses[question.id] !== undefined 
                              ...
                              : 'border-slate-200 bg-slate-50 hover:border-purple-200 hover:shadow-md'
                        }`}
                      >
                              ...
                              <div className="space-y-4">
                                    ...
                                    <label key={value} className="cursor-pointer group">
                                      ...
                                      <div className={`p-2 lg:p-4 text-center rounded-lg lg:rounded-xl border-2 transition-all duration-200 ${
                                        responses[question.id] === value
                                          ...
                                          : 'border-slate-200 bg-white hover:border-purple-300 hover:bg-purple-50 hover:shadow-md group-hover:scale-105'
                                      }`}>
                                ...
                                {responses[question.id] && (
                                  <div className="text-center">
                                    <span className="inline-flex items-center px-3 py-1 lg:px-4 lg:py-2 bg-purple-100 text-purple-800 rounded-full text-xs lg:text-sm font-medium">
                            ...
                            {question.type === "Text" && (
                              <div className="relative">
                                <textarea
                                  ...
                                  className="w-full p-3 lg:p-4 border-2 border-slate-200 rounded-lg lg:rounded-xl focus:border-purple-500 focus:ring-4 focus:ring-purple-100 focus:outline-none resize-none transition-all duration-200 bg-white text-sm lg:text-base"
                                  rows="4"
                  ...
                  <button
                    ...
                    className={`inline-flex items-center px-6 py-3 lg:px-8 lg:py-4 rounded-lg lg:rounded-xl font-semibold text-base lg:text-lg transition-all duration-200 shadow-lg ${
                      submitting
                        ...
                        : 'bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 hover:shadow-xl hover:scale-105 text-white'
                    }`}
                  >
                    {submitting ? (
                      <>
