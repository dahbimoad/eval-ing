...
const TABLE_HEAD = ["Nom", "Prénom", "Filière", "Email", "Modifier", "Supprimer"];
...
function Students() {
  ...
  const [filière, setFilière] = useState(""); // State for filière in modal forms
  const [isDropdownOpen, setIsDropdownOpen] = useState(false); // Manage dropdown visibility

  const filteredRows = filter
    ? TABLE_ROWS.filter((student) => student.filière === filter)
    : TABLE_ROWS;

  const filièreOptions = [
    "GINF-2",
    "GINF-3",
    "GIL-2",
    "GIL-3",
    ...
    <div className="flex h-screen">
      ...
      <div className="h-full w-full overflow-scroll px-6 py-24">
        ...
        <div className="text-end mb-4 flex justify-end items-center gap-4">
          ...
          {/* Dropdown for filtering by filière */}
            ...
            <div className="bg-gray-50 rounded-lg shadow-lg w-full p-6 relative dark:bg-gray-700">
              ...
              <form className="space-y-4 grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    value={filière}
                    onChange={(e) => setFilière(e.target.value)}
                    className="mt-1 block w-full border rounded-md p-2 dark:bg-gray-600 dark:text-white"
                  >
        ...
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table className="w-full table-auto text-center">
            ...
            <tbody>
              {filteredRows.map(({ nom, prénom, filière, email }) => (
                <tr
                  key={nom}
                ...
                >
                  ...
                  <td className="py-4">{filière}</td>
                  <td className="py-4">{email}</td>
                  <td className="py-4 text-green-400">
                    <button
                      onClick={() => {
                        setIsEditOpen(true);
                        setSelectedStudent({ nom, prénom, filière, email });
                      }}
                    >
            ...
            <div className="bg-gray-50 rounded-lg shadow-lg w-full max-w-lg p-6 relative dark:bg-gray-700">
              ...
              <form className="space-y-4 grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    value={filière || selectedStudent?.filière}
                    onChange={(e) => setFilière(e.target.value)}
                    className="mt-1 block w-full border rounded-md p-2 dark:bg-gray-600 dark:text-white"
                  >
                    <option value="">Sélectionner une filière</option>
